...
// mỗi người dùng chỉ thấy đúng dữ liệu của mình. Token được lưu ở localStorage để không
// phải đăng nhập lại mỗi lần mở app (hết hạn sau 30 ngày, xem worker/src/auth.js).
...
// Gọi khi Google trả về credential (ID token) sau khi người dùng chọn tài khoản.
async function handleGoogleCredentialResponse(response) {
  try {
    ...
    updateAccountBox(data.user);
    hideLoginOverlay();
    // Sau khi đăng nhập, tải lại danh sách hội thoại của đúng user này.
...
function initGoogleSignIn() {
  ...
  if (!GOOGLE_CLIENT_ID || GOOGLE_CLIENT_ID.includes('DÁN_CLIENT_ID')) {
    showLoginOverlay('⚠️ Chưa cấu hình GOOGLE_CLIENT_ID trong app.js');
    return;
  }
